<template>
  <!-- 每个小模块的公共样式 -->
  <div class="snail-content-box" :class="[`bg-depth${depth}`, flex && 'flex']">
    <div v-if="showTop" class="top snail-mt-0 snail-flex-no-wrap">
      <n-space class="snail-flex-no-wrap" :size="5">
        <n-ellipsis>
          <n-text>{{ title }}</n-text>
        </n-ellipsis>
        <div class="mt-1">
      ...
      <n-space class="snail-flex-no-wrap" align="center" style="gap: 4px">
        <slot name="top-right"></slot>
        <n-icon v-show="backIcon" size="20" class="snail-cursor-pointer snail-d-block" @click="backHandle">
    ...
    <div v-if="showBottom" class="bottom snail-mt-0">
